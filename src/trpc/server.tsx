...
const createContext = cache(async () => {
  const heads = new Headers(await requestHeaders());
...
>(queryOptionsCb: (api: typeof prefetchApi) => T) {
  ...
  const queryKeyOpts = queryOptions.queryKey?.[1];
  if (
    queryKeyOpts &&
    typeof queryKeyOpts === 'object' &&
    !Array.isArray(queryKeyOpts) &&
    queryKeyOpts.type === 'infinite'
  ) {
